  ...
  return (
    <group position={[1.45, 0, -3.00]}>
      ...
      <mesh position={[-0.98, 1.42, 0.40]}>
        ...
        <meshStandardMaterial color="#563423" roughness={0.88} />
      </mesh>
      {/* Friso frontal da lateral esquerda */}
      <mesh position={[-0.98, 1.42, 0.70]}>
        <boxGeometry args={[0.035, 2.54, 0.02]} />
        <meshStandardMaterial color="#6A422D" roughness={0.85} />
      </mesh>
      ...
      <mesh position={[0, 1.42, 0.12]}>
        ...
        <meshStandardMaterial color="#3D2216" roughness={0.96} />
      </mesh>
      ...
      {BEADBOARD_GROOVES.map((gx) => (
        <mesh key={gx} position={[gx, 1.42, 0.142]}>
          ...
          <meshStandardMaterial color="#2B170E" roughness={0.98} />
        </mesh>
      ))}
      ...
      {SHELVES.map((shelf, idx) => (
        <group key={idx}>
          ...
          <mesh position={[0, shelf.y, 0.40]}>
            ...
            <meshStandardMaterial color="#5E3926" roughness={0.86} />
          </mesh>
          {/* Borda frontal com moldura saliente (bullnose lip) */}
          <mesh position={[0, shelf.lipY, 0.68]}>
            <boxGeometry args={[1.88, 0.07, 0.03]} />
            <meshStandardMaterial color="#70452E" roughness={0.82} />
          </mesh>
          {/* Pinos metálicos de sustentação (detalhe artesanal de latão) */}
          <mesh position={[-0.935, shelf.y - 0.045, 0.40]}>
            <boxGeometry args={[0.018, 0.018, 0.32]} />
            <meshStandardMaterial color="#9C773D" metalness={0.4} roughness={0.5} />
          </mesh>
          <mesh position={[0.935, shelf.y - 0.045, 0.40]}>
            <boxGeometry args={[0.018, 0.018, 0.32]} />
            <meshStandardMaterial color="#9C773D" metalness={0.4} roughness={0.5} />
      ...
      <mesh position={[0, 2.72, 0.40]}>
        ...
        <meshStandardMaterial color="#533120" roughness={0.9} />
      </mesh>
